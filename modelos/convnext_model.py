...
class ConvNeXtModel:
    ...
    def __init__(self):
        self.model = None
        self.img_size = (224, 224)  # tamaño nativo de ConvNeXtTiny
    ...
    def load_labels(self):
        ...
        if os.path.exists(labels_file):
            try:
                ...
                print(f"✅ ConvNeXt: Cargadas {len(labels)} etiquetas desde {labels_file}")
                return labels
            except Exception as e:
                print(f"⚠️ ConvNeXt: Error cargando etiquetas: {e}")
                return {}
        return {}
    ...
    # ============================================================
    # CARGA DE IMÁGENES (sin normalizar: ConvNeXt lo hace internamente)
    # ============================================================
        ...
        for idx, (name, label) in enumerate(labels.items(), 1):
            try:
                ...
                with Image.open(path) as img:
                    ...
                    X.append(np.array(img, dtype=np.float32))  # rango 0-255
                    y.append(int(label) - 1)
            except Exception:
                failed.append(name)
            if idx % 50 == 0:
                print(f"   Cargadas {idx}/{len(labels)} imágenes...")
        print(f"✅ ConvNeXt: {len(X)} imágenes cargadas, {len(failed)} fallaron.")
    ...
    def create_and_train_model(self, labels):
        if not labels or len(labels) < 10:
            print("❌ ConvNeXt: Necesitas al menos 10 imágenes etiquetadas.")
            return False

        X, y = self.load_images_from_labels(labels)
        if len(X) < 10:
            print("❌ ConvNeXt: No hay suficientes imágenes válidas.")
            return False
        ...
        print("\n🏗️ Construyendo ConvNeXtTiny (desde cero)...")
    ...
    def load_or_create_model(self):
        path = "modelos/convnext_model.h5"
        labels = self.load_labels()
        if os.path.exists(path):
            try:
                self.model = keras.models.load_model(path)
                self.is_trained = True
                print("✅ ConvNeXt: Modelo cargado exitosamente.")
                return
            except Exception as e:
                print(f"⚠️ ConvNeXt: Error cargando modelo: {e}")
        if len(labels) >= 10:
            self.create_and_train_model(labels)
        else:
            print("❌ ConvNeXt: No hay suficientes imágenes etiquetadas (mínimo 10).")
    ...
    def predict_image(self, path):
        ...
        try:
            with Image.open(path) as img:
                img = img.convert("RGB").resize(self.img_size)
                arr = np.expand_dims(np.array(img, dtype=np.float32), 0)  # sin /255
            ...
            return {"class": -1, "class_name": f"Error: {e}", "confidence": 0.0, "model": "ConvNeXt"}
    ...
    def analyze_dataset(self, folder="data"):
        ...
        print(f"✅ ConvNeXt: Analizadas {len(results)} imágenes.")
        return results